...
![An image](/images/RD/valid-1.jpg)

- B -->|触发敏感数据操作| C{serviceToken 是否有效?}

![An image](/images/RD/valid-2.jpg)

- C -->|无效| D[发送短信验证码]
- D --> E[用户输入验证码]

![An image](/images/RD/valid-3.jpg)

- E -->|验证成功| F[生成 serviceToken]
- F --> G[允许敏感数据操作]
- C -->|有效| G

![An image](/images/RD/valid-4.jpg)

![An image](/images/RD/valid-5.jpg)
...
- **生成流程：**
  ...
  - 服务端生成随机验证码（如 6 位数字）。
  - 缓存验证码（有效期 5 分钟）并发送至用户绑定手机号。
  - 用户提交验证码后，生成短时效的 serviceToken。
...
2. 进入敏感操作页面时：检查 serviceToken 是否有效。
