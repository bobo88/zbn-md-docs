...
![An image](/images/RD/add-keys.png)

**仓库地址：** `git@gl.szzbn.cn:FE/admin-express.git`
 ...
 * 自动键入通用字段的中间件
 */
const autoFieldsMiddleware = (req: Request, res: Response, next: NextFunction) => {
  // 1. 获取请求中的 deviceId，如果没有则生成新的 UUID
  let deviceId = req.body.deviceId || req.headers['x-device-id'] || req.cookies?.deviceId;
 ...
 * @returns 签名
 */
const generateSign = (appId: string, deviceId: string, deviceName: string, ts: number) => {
