...
- **实现方式：** 通过动态生成令牌来限制请求速率。
  ...
  async function logRequest(ip) {
    ...
    const currentTokens = Math.min(maxTokens, (tokens || maxTokens) + refillRate);
...
4. **判断条件：**
   - 判断每个 IP 的请求次数是否超出限制，比如：如果每分钟请求次数大于某个预设值（如 100 次），则判定该 IP 超限。
