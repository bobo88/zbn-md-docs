...
| Web 标准样式 | 完美支持 CSS3 新特性、Flexbox、Grid | 核心样式兼容性高，但少数 CSS 属性（如 `backdrop-filter`）或高级选择器在 WebView 中可能渲染异常 | 开发后使用 HBuilderX 或 Chrome 远程调试，在 Android 真机上校验样式，针对异常样式通过**条件编译**适配 |
...
3.  **原生差异处理**：针对 App 独有功能（如推送、指纹识别），使用 **条件编译指令** 隔离平台代码：
    1.  App 端专属代码：`#ifdef APP-PLUS ... #endif`
    2.  H5 端排除代码：`#ifndef H5 ... #endif` 避免因平台专属代码导致另一平台运行报错。

## 五、项目 DEMO 截图（部分）

![An image](/images/loT/dog-1.png)

![An image](/images/loT/dog-2.png)

![An image](/images/loT/dog-3.png)
